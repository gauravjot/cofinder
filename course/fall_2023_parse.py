...
import uuid
# Django
from .models import Subjects, InstructionMediums, Instructors, Courses, Locations
...
def parseData(file, termname, termdate):
    ...
    for item in rawDataList:
        ...
        if match:
            ...
            try:
                ...
                if courseSubject.strip() not in [v['name'] for v in subjects]:
                    # Check if this subject is already in database
                    # Check if this subject is already in database
                    try:
                        Subjects.objects.get(id=courseSubject)
            ...
            coursePreReqs = re.sub(
                r"\s+", " ", coursePreReqs.replace("⁇", " ").replace("‼", " "))
            ...
            if (courseName, courseCode.split(" ")[1]) not in [(v['name'], v['code']) for v in courses]:
                ...
                try:
                    courseUUID = Courses.objects.get(
                        name=courseName.strip(), code=courseCode.split(" ")[1].strip()).id
                    ...
                    courses.append(dict(
                        ...
                        prereqs=coursePreReqs.replace("Prerequisite(s):", "").split(
                            "Corequisite(s):")[0].replace("±", " ").strip(),
                        coreqs=coursePreReqs.split("orequisite(s):")[1].replace(
                            "±", " ").replace("Pre- or c", "").strip() if "corequisite(s):" in coursePreReqs else "",
                        note=""))
            else:
                courseUUID = next(
            ...
            for sect in sects:
                bb = sect.strip()
                sect = sect.replace("UFV⁇", "UFV±± ", 1).replace("M⁇", "M", 1).replace("T⁇", "T", 1).replace("W⁇", "W", 1).replace(
                    "R⁇", "R", 1).replace("F⁇", "F", 1).replace("S⁇", "S", 1).replace("⁇⁇⁇⁇ ONLINE UFV", "⁇⁇±⁇ONLINE UFV")
                ...
                if matches:
                    for match in matches:
                        ...
                        if sectionDict['sectionProf'] not in [v['name'] for v in instructors]:
                            # Check if this course is already in database
                            try:
                                instructorUUID = Instructors.objects.get(
                                    name=sectionDict['sectionProf'].strip()).id
                        ...
                        for sch in match[3].split("\n"):
                            ...
                            if len(subSectDict['location']) > 0:
                                ...
                                try:
                                    ...
                                    try:
                                        locationUUID = Locations.objects.get(campus=campus,
                                                                             building=building,
                                                                             room=room).id
                                    ...
                                    for d in list(subSectDict['days']):
                                        schedules.append(dict(
                                            ...
                                            date_start=datetime.datetime.strptime(
                                                (subSectDict['date'].split(" ")[0]), '%d-%b-%Y').strftime('%Y%m%d') if len(subSectDict['date'].split(" ")[0]) > 0 else "",
                                            date_end=datetime.datetime.strptime(
                                                (subSectDict['date'].split(" ")[1]), '%d-%b-%Y').strftime('%Y%m%d') if len(subSectDict['date'].split(" ")) > 1 and len(subSectDict['date'].split(" ")[1]) > 0 else "",
                                            is_weekly=len(subSectDict['date'].split(" ")) > 1 and (subSectDict['date'].split(" ")[0] != subSectDict['date'].split(" ")[1])
                                        ))
                                else:
                                    schedules.append(dict(
                                        id=uuid.uuid4(),
                        ...
                        if cleansedMedium not in [v['name'] for v in mediums]:
                            # Check if this course is already in database
                            try:
                                mediumUUID = InstructionMediums.objects.get(
                                    name=cleansedMedium).id
                            except InstructionMediums.DoesNotExist:
    ...
    return insertToDatabase(result, termdate=termdate, termname=termname, failed=failed)
